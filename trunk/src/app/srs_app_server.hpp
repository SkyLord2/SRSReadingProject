...
public:
    ...
    /* 监听 rtmp/http api/http stream 的请求*/
...
public:
    ...
    // SrsTcpListener::cycle()监听协程接收到一条新连接后调用SrsServer::accept_client()函数的处理过程
...
/*
SRS4.0 RTMP服务模块对应的类对象是SrsServerAdapter，
SrsServerAdapter有SrsServerAdapter::initialize()和SrsServerAdapter::run()两个入口函数，
RTMP服务模块的启动工作主要在SrsServerAdapter::run()接口中完成。
*/
